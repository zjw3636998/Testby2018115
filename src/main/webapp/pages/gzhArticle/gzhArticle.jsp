		...
		<script type="text/javascript">  
			...
			function firstPage(){  
			    if(currentPage == 1){  
			        ...
			        submitForm("<%=context%>/searchArticle.do?pageNum=1");  
			        return true;  
			    }  
			}  
			// 下一页  
			function nextPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/searchArticle.do?pageNum=" + (currentPage+1));  
			        return true;  
			    }  
			}  
			// 上一页  
			...
			function lastPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/searchArticle.do?pageNum=${totalPage}");  
			            return true;  
			        }  
			    }  
		</script>
...
<body>
	...
	<div class="allmebWrap">
				<div class="weadmin-block i-block">
					<i class="layui-icon"><a href="pages/gzhArticle/addArticle.html" class="i-a-block">添加策略</a></i>
				</div>
		<div class="layui-tab-item layui-show ">
			<table class="layui-table i-table">
				...
				<tbody>
					<c:forEach var="articleList" items="${requestScope['articleList']}">
						<tr>
								<td>${articleList['article_title'] }</td>
								<td>${articleList['article_author'] }</td>
								<td>${articleList['article_time'] }</td>
								<td>${articleList['article_reader'] }</td>
								<td>
									<a href="delArticle.do?article_id=${articleList['article_id']}" onclick="return confirm('确认要删除此策略文章？')">删除</a>
									<a onclick="WeAdminShow('查看内容','searchContent.do?article_id=${articleList['article_id']}')">详情</a>
								</td>
						</tr>
					</c:forEach>
				</tbody>
