		...
		<script type="text/javascript">  
			...
			function firstPage(){  
			    if(currentPage == 1){  
			        ...
			        submitForm("<%=context%>/searchDelArticle.do?pageNum=1");  
			        return true;  
			    }  
			}  
			// 下一页  
			function nextPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/searchDelArticle.do?pageNum=" + (currentPage+1));  
			        return true;  
			    }  
			}  
			// 上一页  
			...
			function lastPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/searchDelArticle.do?pageNum=${totalPage}");  
			            return true;  
			        }  
			    }  
		</script>
				...
				<tbody>
					<c:forEach var="articleList" items="${requestScope['articleList']}">
						<tr>
								...
								<td>
									<a href="completelyDeleted.do?article_id=${articleList['article_id']}" onclick="return confirm('确认要删除此策略文章？')">删除</a>
									<a href="recover.do?article_id=${articleList['article_id']}">恢复</a>
								</td>
						</tr>
					</c:forEach>
				</tbody>
