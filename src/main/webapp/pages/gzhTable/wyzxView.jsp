		...
		<script type="text/javascript">  
			...
			function firstPage(){  
			    if(currentPage == 1){  
			        ...
			        submitForm("<%=context%>/selYyzx.do?orderType=2&pageNum=1");  
			        return true;  
			    }  
			}  
			// 下一页  
			function nextPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/selYyzx.do?orderType=2&pageNum=" + (currentPage+1));  
			        return true;  
			    }  
			}  
			// 上一页  
			...
			function lastPage(){  
			    if(currentPage == totalPage){ 
			        ...
			        submitForm("<%=context%>/selYyzx.do?orderType=2&pageNum=${totalPage}");  
			            return true;  
			        }  
			    }  
		</script>
				...
				<table class="layui-table i-table">
					...
					<c:forEach var="orderList" items="${requestScope['orderList']}">
						<tr>
								...
								<td>
									<c:choose>
										<c:when test="${empty orderList['order_deal']}">
											<button onclick="dealEmp('${orderList.order_id}');" style="background-color: #fff;border: none;outline: none;">负责此单</button>
										</c:when>
										<c:otherwise>${orderList['order_deal']}</c:otherwise>
									</c:choose>	
								</td>
								<c:choose>
										<c:when test="${orderList['order_deal']==sessionScope.employee.empName&&orderList['order_state']==3}">
											<td>${orderList['openid'] }</td>
											<td>${orderList['out_trade_no'] }</td>
											<td>
												<a onclick="WeAdminShow('备注','pages/xcxUser/remakeContent.jsp',600,400)">查看备注</a>
											</td>
										</c:when>
										<c:otherwise>
											<td>*************</td>
	...
	<script type="text/javascript">
		$(function() {
			$('#searchInput').click(function() {
				...
				window.location.href="selYyzx.do?orderType=2&searchType="+param1+"&searchContent="+encodeURI(encodeURI(param2));
				}else{
					alert('请选择搜索类型！');
				}
			});
			...
			if(dealBoolean){
				$.ajax({
	                ...
	                success: function (data){
	                   if(data.success==200){
	                      alert("申请成功！");
	                      window.location.href="<%=context%>/selYyzx.do?orderType=2";
	                   }else{
	                	   alert("申请失败!");
	                	   window.location.href="<%=context%>/selYyzx.do?orderType=2";
	                   }
