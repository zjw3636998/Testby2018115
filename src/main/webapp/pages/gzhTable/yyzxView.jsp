		...
		<script type="text/javascript">  
			...
			function firstPage(){  
			    if(currentPage == 1){  
			        ...
			        submitForm("<%=context%>/selYyzx.do?orderType=1&pageNum=1");  
			        return true;  
			    }  
			}  
			// 下一页  
			function nextPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/selYyzx.do?orderType=1&pageNum=" + (currentPage+1));  
			        return true;  
			    }  
			}  
			// 上一页  
			...
			function lastPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/selYyzx.do?orderType=1&pageNum=${totalPage}");  
			            return true;  
			        }  
			    }  
		</script>
			...
			<table class="layui-table i-table">
				...
				<c:forEach var="orderList" items="${requestScope['orderList']}">
					<tr>
							...
							<c:choose>
									<c:when test="${orderList['order_deal']==sessionScope.employee.empName}">
		...
		$(function() {
			$('#searchInput').click(function() {
				...
				if(param1!=""){
					...
					window.location.href="selYyzx.do?orderType=1&searchType="+param1+"&searchContent="+encodeURI(encodeURI(param2))
				}else{
					alert('请选择搜索类型！');
				}
			...
			if(dealBoolean){
				$.ajax({
	                ...
	                success: function (data){
	                   if(data.success==200){
	                      alert("负责成功！");
	                      window.location.href="<%=context%>/selYyzx.do?orderType=1";
	                   }else{
	                	   alert("负责失败!");
	                	   window.location.href="<%=context%>/selYyzx.do?orderType=1";
	                   }
