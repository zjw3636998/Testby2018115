		...
		<script type="text/javascript">  
			...
			function firstPage(){  
			    if(currentPage == 1){  
			        ...
			        submitForm("<%=context%>/selFile.do?pageNum=1&fileType=2");  
			        return true;  
			    }  
			}  
			// 下一页  
			function nextPage(){  
			    if(currentPage == totalPage){  
			        ...
			        submitForm("<%=context%>/selFile.do?pageNum=" + (currentPage+1)+"&fileType=2");  
			        return true;  
			    }  
			}  
			// 上一页  
			...
			function lastPage(){  
			    if(currentPage == totalPage){
			        ...
			        submitForm("<%=context%>/selFile.do?pageNum=${totalPage}&fileType=2");  
			            return true;  
			        }  
			    }  
		</script>
...
<body>
	<div class="allmebWrap">
				...
				<div class="weadmin-block i-block">
						<button class="layui-btn" onclick="WeAdminShow('添加用户','pages/fileTable/addCompanyFile.html',650,500)"><i class="layui-icon"></i>添加</button>
				</div>
				<div class="i-r-nav">
				    	<div class="i-search">
							...
							<c:forEach var="fileList" items="${requestScope['fileList']}">
								<tr>
										...
										<td>
											...
											<a href="deleteFile.do?fileUrl=${fileList['file_url']}&fileType=2" onclick="return confirm('确认要删除此文件？')">删除</a>
										</td>
								</tr>
							</c:forEach>
							</tbody>
			...
			$(function() {
				$('#searchInput').click(function() {
					...
					if(param1!=""){
						...
						window.location.href="selFile.do?searchType="+param1+"&searchContent="+encodeURI(encodeURI(param2))+"&fileType=2"
					}else{
						alert('请选择搜索类型！');
					}
